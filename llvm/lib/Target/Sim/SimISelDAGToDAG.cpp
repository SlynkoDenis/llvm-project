...
public:
  explicit SimDAGToDAGISel(SimTargetMachine &tm) : SelectionDAGISel(tm) {}
...
SDNode* SimDAGToDAGISel::getGlobalBaseReg() {
...
bool SimDAGToDAGISel::tryInlineAsm(SDNode *N){
  ...
  for(unsigned i = 0, e = N->getGluedNode() ? NumOps - 1 : NumOps; i < e; ++i) {
    ...
    if ((!IsTiedToChangedOp && (!HasRC || RC != SIM::IntRegsRegClassID))
        || NumRegs != 2)
      continue;
    ...
    if (Kind == InlineAsm::Kind_RegDef ||
      ...
      Register GPVR = MRI.createVirtualRegister(&SIM::IntPairRegClass);
      ...
      SDValue Sub0 = CurDAG->getTargetExtractSubreg(SIM::sub_even, dl, MVT::i32,
                                                    RegCopy);
      SDValue Sub1 = CurDAG->getTargetExtractSubreg(SIM::sub_odd, dl, MVT::i32,
                                                    RegCopy);
      ...
      SDValue Pair = SDValue(
          CurDAG->getMachineNode(
              ...
              {
                  CurDAG->getTargetConstant(SIM::IntPairRegClassID, dl,
                                            MVT::i32),
                  T0,
                  CurDAG->getTargetConstant(SIM::sub_even, dl, MVT::i32),
                  T1,
                  CurDAG->getTargetConstant(SIM::sub_odd, dl, MVT::i32),
              }),
          0);
    ...
    if(PairedReg.getNode()) {
      ...
      if (IsTiedToChangedOp)
        ...
        Flag = InlineAsm::getFlagWordForRegClass(Flag, SIM::IntPairRegClassID);
...
//       TopPart = SDValue(CurDAG->getMachineNode(SIM::SRAri, dl, MVT::i32, DivLHS,
//                                    CurDAG->getTargetConstant(31, dl, MVT::i32)),
//                         0);
//     } else {
//       TopPart = CurDAG->getRegister(SIM::G0, MVT::i32);
//     }
//     TopPart = CurDAG->getCopyToReg(CurDAG->getEntryNode(), dl, SIM::Y, TopPart,
//                                    SDValue())
//                   .getValue(1);

//     // FIXME: Handle div by immediate.
//     unsigned Opcode = N->getOpcode() == ISD::SDIV ? SIM::SDIVrr : SIM::UDIVrr;
...
/// inline asm expressions.
bool
SimDAGToDAGISel::SelectInlineAsmMemoryOperand(const SDValue &Op,
                                                unsigned ConstraintID,
