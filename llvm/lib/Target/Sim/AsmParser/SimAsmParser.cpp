//===-- SimAsmParser.cpp - Parse Sim assembly to MCInst instructions --===//
//
...
class SimOperand : public MCParsedAsmOperand {
  enum OperandKind {
    // KindInvalid,
    OpKind_Token,
    OpKind_Reg,
...
public:
  SimOperand(OperandKind kind, SMLoc startLoc, SMLoc endLoc)
  ...
  static std::unique_ptr<SimOperand> createToken(StringRef Str, SMLoc Loc) {
    auto Op = std::make_unique<SimOperand>(OpKind_Token, Loc, Loc);
    Op->Token.Data = Str.data();
    Op->Token.Length = Str.size();
    return Op;
  }

  static std::unique_ptr<SimOperand> createReg(unsigned Num, SMLoc StartLoc,
                                                SMLoc EndLoc) {
    auto Op = std::make_unique<SimOperand>(OpKind_Reg, StartLoc, EndLoc);
    Op->Reg.RegNo = Num;
    return Op;
  }

  static std::unique_ptr<SimOperand> createImm(const MCExpr *Expr,
                                                SMLoc StartLoc, SMLoc EndLoc) {
    auto Op = std::make_unique<SimOperand>(OpKind_Imm, StartLoc, EndLoc);
    Op->Imm = Expr;
    return Op;
  }
...
class SimAsmParser : public MCTargetAsmParser {
// Auto-generated instruction matching functions
#define GET_ASSEMBLER_HEADER
#include "SimGenAsmMatcher.inc"
...
public:
  SimAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
// TBD
bool SimAsmParser::ParseDirective(AsmToken DirectiveID) { return true; }

bool SimAsmParser::ParseInstruction(ParseInstructionInfo &Info, StringRef Name,
                                     SMLoc NameLoc, OperandVector &Operands) {
  // First operand in MCInst is instruction mnemonic.
  Operands.push_back(SimOperand::createToken(Name, NameLoc));
...
bool SimAsmParser::parseOperand(OperandVector &Operands, StringRef Mnemonic) {
  ...
  if (Lexer.is(AsmToken::Percent)) {
    ...
    if (ParseRegister(RegNo, StartLoc, EndLoc))
      return true;
    Operands.push_back(SimOperand::createReg(RegNo, StartLoc, EndLoc));
    return false;
  }
  ...
  if (Lexer.is(AsmToken::Integer)) {
    ...
    Operands.push_back(SimOperand::createImm(Expr, StartLoc, EndLoc));
    return false;
  }
...
OperandMatchResultTy SimAsmParser::parseImmWO(OperandVector &Operands) {
...
bool SimAsmParser::ParseRegister(unsigned &RegNo, SMLoc &StartLoc,
...
OperandMatchResultTy SimAsmParser::tryParseRegister(unsigned &RegNo,
                                                     SMLoc &StartLoc,
                                                     SMLoc &EndLoc) {
  bool Result =
...
bool SimAsmParser::MatchAndEmitInstruction(SMLoc IdLoc, unsigned &Opcode,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      // TODO
      // ErrorLoc = ((SimOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IdLoc;
    }
  ...
  case Match_MnemonicFail: {
    ...
    SimOperand &Op = static_cast<SimOperand &>(*Operands[0]);
    std::string Suggestion = SimMnemonicSpellCheck(Op.getToken(), FBS, 0);
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeSimAsmParser() {
  RegisterMCAsmParser<SimAsmParser> x(getTheSimTarget());
}
